import React from 'react';

export default function Registry() {
    return (
        <div>
            <h1>Registry</h1>
            <p>
                While we would be more than appreciative of gifts that would go towards our honey moon in New Zealand, exploring the realm of Middle-Earth,
                we don't want you to feel obligated to have to give us anything. We are more happy to have you join us on our special day.
            </p>
            <p><a href="https://venmo.com/?txn=pay&audience=public&recipients=Jeremy-Wenzel&amount=0">Venmo</a></p>
            <p><a href="https://paypal.me/jwenzel15?country.x=US&locale.x=en_US">PayPal</a></p>
            <p>Checks and cash are also welcome, and can be left at the venue.</p>
        </div>
    )
}